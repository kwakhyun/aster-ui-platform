...
의료미용 제품군을 가정한 개인 PoC로, Figma 토큰 변경을 React API와 세 소비 앱에 대조하고 사람 검토와 릴리스 리허설까지 확인하는 Studio를 구축했습니다.

다음 질문을 실제 화면과 검증 명령으로 확인하는 것이 구현 목표였습니다.
...
### 1. 디자인 변경 검토를 Studio의 중심에 둔다

Studio의 중심에는 선택한 컴포넌트의 현재 상태와 릴리스 준비 근거를 배치했습니다. 도입 현황, Figma 일치, 버전 호환성, 문서 최신화, 사람 검토를 컴포넌트 미리보기와 함께 확인할 수 있습니다.

Studio는 공용 패키지에서 생성한 매니페스트로 컴포넌트 6개를 읽고, 선택한 컴포넌트의 미리보기와 API 계약을 보여줍니다.

### 2. Figma 별칭을 코드 계약과 대조한다
...
| AI | Claude Code 구조화 제안 래퍼, 제안 검증, 변조 및 경로 이탈 차단, 제한 시간, 제안부터 별도 승인 기록까지의 전체 흐름 검증 |
